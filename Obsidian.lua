...
local Library = {
    ...
    PillCorners = {},
    --// [line] = ApplyGeometry, direfresh saat CornerRadius berubah
    RunningLines = {},
...
-- ========================================================================== --
--                      RUNNING LINE BORDER ANIMATION                         --
-- ========================================================================== --

function Library:AddRunningLineBorder(Parent, Config)
    Config = Config or {}
    
    local LineColor = Config.Color or "AccentColor" -- Default to theme
    local LineHeight = Config.Height or 2 -- Tinggi garis (thickness)
    local AnimationSpeed = Config.Speed or 2.0 -- Durasi animasi gradient
    local UseRainbow = Config.UseRainbow or false -- Rainbow effect
    ...
    assert(
        ...
        "AddRunningLineBorder expects a GuiObject as Parent, got: " .. typeof(Parent)
    )

    local RunService = game:GetService("RunService")

    -- Garis diposisikan relatif terhadap *content box* parent, yaitu area yang
    -- sudah dikurangi UIPadding.  Parent yang ber-padding (mis. Label milik
    -- AddDraggableLabel) karena itu perlu dikompensasi balik, kalau tidak garis
    -- akan ter-inset sejauh padding-nya dan tidak konsisten dengan parent yang
    -- tidak ber-padding (Button / Holder).
    --
    -- Inset horizontalnya sendiri diturunkan dari corner radius, bukan angka
    -- hardcoded: garis lurus di dekat sisi bawah frame yang sudutnya membulat
    -- akan menyembul keluar siluet kalau inset-nya lebih kecil dari lengkung
    -- sudut.  BottomGap dijaga konstan berapa pun LineHeight-nya.
    local BottomGap = 2
    ...
    local function GetPadding()
        ...
        return Padding.PaddingLeft.Offset, Padding.PaddingRight.Offset, Padding.PaddingBottom.Offset
    end

    -- Inset minimum agar ujung garis tetap berada di dalam sudut membulat.
    -- Untuk radius R dan jarak d dari sisi bawah: R - sqrt(R^2 - (R-d)^2).
    local function GetCornerInset()
        local Radius = Library.CornerRadius or 0
        if Radius <= 0 then
            return 0
        end

        local Delta = Radius - BottomGap
        if Delta <= 0 then
            return 0
        end

        return Radius - math.sqrt(math.max(Radius * Radius - Delta * Delta, 0))
    end
    ...
    -- Dipanggil ulang setiap kali padding atau corner radius berubah, sehingga
    -- garis tetap sejajar setelah SetIcon() atau Window:SetCornerRadius().
    local function ApplyGeometry()
        local PadLeft, PadRight, PadBottom = GetPadding()
        local Inset = math.ceil(GetCornerInset())

        line.Size = UDim2.new(1, PadLeft + PadRight - (Inset * 2), 0, LineHeight)
        line.Position = UDim2.new(0, Inset - PadLeft, 1, PadBottom - LineHeight - BottomGap)
    end

    ApplyGeometry()

    -- SetIcon() mengubah PaddingLeft/PaddingRight saat runtime, dan
    -- Window:SetCornerRadius() mengubah radius.  Daftarkan garis supaya
    -- geometrinya ikut menyesuaikan alih-alih tertinggal di nilai awal.
    local PaddingConnection
    do
        local Padding = Parent:FindFirstChildOfClass("UIPadding")
        if Padding then
            PaddingConnection = Padding.Changed:Connect(ApplyGeometry)
            Library:GiveSignal(PaddingConnection)
        end
    end

    Library.RunningLines[line] = ApplyGeometry

    local function Unregister()
        Library.RunningLines[line] = nil
    end

    -- Set color (theme or custom).  Saat UseRainbow aktif warnanya di-drive
    -- per frame, jadi jangan didaftarkan ke registry tema supaya keduanya tidak
    -- saling menimpa.
    if UseRainbow then
        line.BackgroundColor3 = Color3.fromHSV(0, 1, 1)
    elseif typeof(LineColor) == "Color3" then
    ...
    -- Add animated gradient that moves horizontally
    local gradient = New("UIGradient", {
        ...
        Transparency = NumberSequence.new({
            NumberSequenceKeypoint.new(0, 0.7),    -- Semi-transparent
            NumberSequenceKeypoint.new(0.3, 0),    -- Fully visible
            NumberSequenceKeypoint.new(0.7, 0),    -- Fully visible
            NumberSequenceKeypoint.new(1, 0.7),    -- Semi-transparent
        }),
        Offset = Vector2.new(0, 0), -- Start position
        Rotation = 0, -- Horizontal gradient
        Parent = line,
    })
    
    -- Animate gradient offset for "running" effect
    ...
    local connection = RunService.RenderStepped:Connect(function(dt)
        ...
        offset = offset + (dt / AnimationSpeed)
        if offset >= 1 then
            offset = 0 -- Loop
        end

        -- Move gradient horizontally (creates running effect)
        gradient.Offset = Vector2.new(offset * 2 - 1, 0) -- -1 to 1

        if UseRainbow then
            hue = (hue + dt / AnimationSpeed) % 1
            line.BackgroundColor3 = Color3.fromHSV(hue, 1, 1)
        end
    end)
    ...
    -- Return controller.
    --
    -- Controller ini tabel closure biasa, jadi pemanggilan yang benar memakai
    -- titik: RunningLine.SetColor(c).  Karena seluruh API library lain memakai
    -- titik-dua, salah-pakai jadi jebakan yang mudah — dulu `:SetColor(c)`
    -- mengirim tabel controller sebagai argumen Color dan gagal tanpa suara.
    -- Resolve() membuat kedua gaya bekerja.
    local Controller = {}

    local function Resolve(First, Second)
    ...
    function Controller.Stop()
        ...
        if PaddingConnection then
            PaddingConnection:Disconnect()
            PaddingConnection = nil
        end
        Unregister()
        if line then
            line:Destroy()
    ...
    function Controller.SetColor(First, Second)
        local Color = Resolve(First, Second)
        if typeof(Color) ~= "Color3" then
            warn("RunningLine.SetColor expects a Color3, got: " .. typeof(Color))
            return
        end
        if line and line.Parent then
            UseRainbow = false -- warna eksplisit mengalahkan rainbow
            -- Lepas dari registry tema, kalau tidak warna ini akan ditimpa lagi
            -- pada perubahan tema berikutnya.
            Library:RemoveFromRegistry(line)
            line.BackgroundColor3 = Color
        end
    end

    function Controller.SetSpeed(First, Second)
        local Speed = Resolve(First, Second)
        if type(Speed) ~= "number" or Speed <= 0 then
            warn("RunningLine.SetSpeed expects a positive number, got: " .. tostring(Speed))
            return
        end
        AnimationSpeed = Speed
    end

    Library:GiveSignal(line.Destroying:Connect(function()
        animating = false
        if connection then
            connection:Disconnect()
            connection = nil
...
function Library:AddDraggableLabel(...)
    ...
    -- Add running line border (default: AccentColor, theme-synced)
    DraggableLabel.RunningLine = Library:AddRunningLineBorder(Label, {
        Color = "AccentColor",
        Enabled = true,
    })
...
function Library:AddDraggableButton(...)
    ...
    DraggableButton:SetText(tostring(Text or ""))
    ...
    -- Add running line border (default: AccentColor, theme-synced)
    DraggableButton.RunningLine = Library:AddRunningLineBorder(Button, {
        Color = "AccentColor",
        Enabled = true,
    })
...
function Library:AddDraggableMenu(Name: string)
    if type(Name) ~= "string" then
        Name = tostring(Name or "Draggable Menu")
    end
    
    local Holder = New("Frame", {
        AutomaticSize = Enum.AutomaticSize.Y, -- Only auto-size height, not width
        BackgroundColor3 = "BackgroundColor",
        Position = UDim2.fromOffset(6, 6),
        Size = UDim2.fromOffset(250, 0), -- Fixed width of 250px
        ZIndex = 10,
        Parent = ScreenGui,
    })
    table.insert(
    ...
    -- Add running line border (default: AccentColor, theme-synced).
    -- Dikembalikan sebagai nilai ketiga supaya bisa dikustomisasi atau dihapus
    -- lewat :Stop(); pemanggil lama yang hanya mengambil dua nilai tidak
    -- terpengaruh.
    local MenuRunningLine = Library:AddRunningLineBorder(Holder, {
        Color = "AccentColor",
        Enabled = true,
    })

    return Holder, Container, MenuRunningLine
end
...
function Library:CreateArrayList(Config)
    Config = Config or {}
    
    local Position = Config.Position or "TopRight" -- TopRight, TopLeft, BottomRight, BottomLeft
    local Width = Config.Width or 200
    local FontSize = Config.FontSize or 14
    local Font = Config.Font or Enum.Font.Code
    local RainbowSpeed = Config.RainbowSpeed or 0.1
    local RainbowSaturation = Config.RainbowSaturation or 0.6
    local RainbowBrightness = Config.RainbowBrightness or 1
    local TextColor = Config.TextColor or Color3.fromRGB(255, 255, 255)
    local ShowColorLine = Config.ShowColorLine ~= false -- default true
    local ColorLineWidth = Config.ColorLineWidth or 2
    local ItemSpacing = Config.ItemSpacing or 2
    local OffsetX = Config.OffsetX or 10
    local OffsetY = Config.OffsetY or 10
    local LowercaseText = Config.LowercaseText ~= false -- default true
    local SortByLength = Config.SortByLength ~= false -- default true
    local TextStroke = Config.TextStroke ~= false -- default true
    ...
    local function GetPositionUDim2()
        if Position == "TopRight" then
            return UDim2.new(1, -(Width + OffsetX), 0, OffsetY)
        elseif Position == "TopLeft" then
            return UDim2.new(0, OffsetX, 0, OffsetY)
        else
            return UDim2.new(1, -(Width + OffsetX), 0, OffsetY) -- default TopRight
        end
    end
    
    local function GetHorizontalAlignment()
        if Position == "TopRight" then
    ...
    local function CreateUI()
        if ArrayList.ListFrame and ArrayList.ListFrame.Parent then
            ArrayList.ListFrame:Destroy()
            ArrayList.ListFrame = nil
        end
        
        -- Simple size for top positions only
        local frameSize = UDim2.new(0, Width, 1, -(OffsetY * 2))
        
        -- Create Frame directly in Library's ScreenGui
        ArrayList.ListFrame = New("Frame", {
            Size = frameSize,
            Position = GetPositionUDim2(),
            BackgroundTransparency = 1,
            ZIndex = 1000,
        ...
        local layout = New("UIListLayout", {
            ...
            Parent = ArrayList.ListFrame,
        })
        
        Library:GiveSignal(ArrayList.ListFrame.Destroying:Connect(function()
            if ArrayList.UpdateConnection then
                ArrayList.UpdateConnection:Disconnect()
                ArrayList.UpdateConnection = nil
            end
    ...
    local function UpdateList(Force)
        ...
        if Library.Toggles then
            for toggleIdx, toggle in pairs(Library.Toggles) do
                if toggle.Value and toggle.Text and type(toggle.Text) == "string" then
                    -- Filter out internal UI toggles
                    if toggleIdx ~= "KeybindMenuOpen" 
                        and toggleIdx ~= "ShowCustomCursor" 
                        and toggleIdx ~= "EnableNotifications" 
                        and toggleIdx ~= "ShowArrayList"
                        and toggleIdx ~= "AlwaysOnTop"
                        and toggleIdx ~= "WindowGlow"
                        and toggleIdx ~= "AnimationsEnabled" then
        ...
        for i, feature in ipairs(activeFeatures) do
            ...
            local displayText = LowercaseText and string.lower(feature.name) or feature.name
            
            local textLabel = New("TextLabel", {
                Size = UDim2.new(1, ShowColorLine and -(ColorLineWidth + 6) or -6, 1, 0),
                BackgroundTransparency = 1,
                Text = displayText,
                TextColor3 = TextColor,
                Font = Font,
                TextSize = FontSize,
                TextXAlignment = GetHorizontalAlignment() == Enum.HorizontalAlignment.Right 
                    and Enum.TextXAlignment.Right 
                    or Enum.TextXAlignment.Left,
                TextStrokeTransparency = TextStroke and 0.5 or 1,
                Parent = rowFrame,
            })
            
            if ShowColorLine then
                local colorLine = New("Frame", {
                    Size = UDim2.new(0, ColorLineWidth, 1, 0),
                    Position = GetHorizontalAlignment() == Enum.HorizontalAlignment.Right 
                        and UDim2.new(1, -ColorLineWidth, 0, 0)
    ...
    function ArrayList:SetPosition(NewPosition)
        if ArrayList.Position == NewPosition then
            return
        end
        
        -- Only accept TopRight or TopLeft
        if NewPosition ~= "TopRight" and NewPosition ~= "TopLeft" then
        ...
        if ArrayList.Enabled and ArrayList.ListFrame then
            ArrayList.ListFrame.Position = GetPositionUDim2()
            ArrayList.ListFrame.Size = UDim2.new(0, Width, 1, -(OffsetY * 2))
            
            local layout = ArrayList.ListFrame:FindFirstChildOfClass("UIListLayout")
            if layout then
                layout.HorizontalAlignment = GetHorizontalAlignment()
    ...
    function ArrayList:SetVisible(Visible)
        if ArrayList.ListFrame then
            ArrayList.ListFrame.Visible = Visible
        end
    end
...
do
    ...
    function Funcs:AddInput(Idx, Info)
        ...
        local Input = {
            ...
            Text = Info.Text,
            Value = type(Info.Default) == "string" and Info.Default or tostring(Info.Default or ""),
...
function Library:CreateWindow(WindowInfo)
    ...
    do
        ...
        local function AddFooterSegment(Info)
            ...
            local Label = New("TextLabel", {
                ...
                Size = UDim2.new(0, 0, 1, 0),
                Text = Text,
                TextColor3 = Copyable and "AccentColor" or "FontColor",
                TextSize = 14,
    ...
    function Window:SetCornerRadius(Radius: number)
        ...
        -- Inset garis running line diturunkan dari CornerRadius, jadi harus
        -- dihitung ulang setelah nilainya berubah.
        for Line, ApplyGeometry in Library.RunningLines do
            if Line.Parent then
                ApplyGeometry()
            else
                Library.RunningLines[Line] = nil
    ...
    -- Menaikkan DisplayOrder ScreenGui library supaya menang render terhadap
    -- GUI lain di game.  Nilai dasarnya 998 (lihat pembuatan Library.ScreenGui).
    function Window:SetAlwaysOnTop(Value: boolean?)
        Value = Value == true
        WindowInfo.AlwaysOnTop = Value
        Window.AlwaysOnTop = Value

        if Library.ScreenGui then
            Library.ScreenGui.DisplayOrder = Value and 10000 or 998
        end
    ...
    function Window:AddTab(...)
        ...
        do
            ...
            if Icon then
                ...
                -- Simpan string scheme key bukan Color3 value agar ikuti theme changes
                local registryColor = (typeof(TabIconColor) == "string") and TabIconColor or ResolvedIconColor
                if registryColor then
                    Library.Registry[TabIcon] = { ImageColor3 = registryColor }
                end
            end
        ...
        local function RenderUserPanelInformation(Information)
            ...
            -- Reuse label yang sudah ada kalau jumlah barisnya sama.  Pemanggil
            -- berkala (mis. jam yang diperbarui per detik) jadi cukup mengganti
            -- Text, tanpa destroy/create instance setiap kali.
            if #Rows == #UserPanelInformationLabels then
                local Reusable = true
                for _, Label in ipairs(UserPanelInformationLabels) do
                ...
                if Reusable then
                    for Index, Row in ipairs(Rows) do
                        UserPanelInformationLabels[Index].Text = "• " .. tostring(Row)
                    end
                    return
                end
            end
        ...
        -- Memperbarui hanya baris Information, tanpa menyentuh avatar.
        -- Tab:UserPanelBox() memanggil RenderUserPanelAvatar() juga, sehingga
        -- memakainya untuk update berkala (mis. jam) akan me-refetch thumbnail
        -- setiap kali.  Pakai ini untuk itu.
        function Tab:SetUserPanelInformation(Information)
            local State = Tab.UserPanelState
            State.Defined = true
    ...
    local function CreateSquareMobileButton(IconName)
        ...
        -- Custom logo untuk toggle button (menu)
        if IconName == "menu" then
            BtnIcon.Image = "rbxassetid://121546427274411"
            BtnIcon.ImageRectOffset = Vector2.zero
            BtnIcon.ImageRectSize = Vector2.zero
        end
    ...
    ToggleButton:SetIconColor(Library.Scheme.AccentColor)
    Library.Registry[ToggleButton.Icon] = { ImageColor3 = "AccentColor" }
    ToggleButton.Button.MouseButton1Click:Connect(function()
        Library:Toggle()
    end)

    LockButton = CreateSquareMobileButton("lock")
    LockButton:SetIconColor(Library.Scheme.DestructiveColor)
    Library.Registry[LockButton.Icon] = { ImageColor3 = "DestructiveColor" }
    LockButton.Button.MouseButton1Click:Connect(function()
        Library.CantDragForced = not Library.CantDragForced
        if Library.CantDragForced then
            -- Locked: pakai skema warna yang tersedia (tidak hardcode)
            LockButton:SetIconColor(Library.Scheme.AccentColor)
            Library.Registry[LockButton.Icon] = { ImageColor3 = "AccentColor" }
        else
            LockButton:SetIconColor(Library.Scheme.DestructiveColor)
            Library.Registry[LockButton.Icon] = { ImageColor3 = "DestructiveColor" }
        end
    ...
    TrashButton:SetIconColor(Library.Scheme.DestructiveColor)
    Library.Registry[TrashButton.Icon] = { ImageColor3 = "DestructiveColor" }
    ...
    if WindowInfo.ShowMobileButtons == false then
        ...
        -- Selalu sembunyikan lock dan trash, hanya logo toggle yang muncul
        LockButton.Button.Visible = false
        TrashButton.Button.Visible = false
    end
